function ubbEditor(_id)
{
  ...
  this.tToolbarSets['default'] = ['Source','separator','Bold','Underline','separator','Link','Unlink','Image'];
  ...
  this.tToolbarSets['mini'] = ['Bold','Underline','separator','Link','Unlink'];
  ...
  this.tGetSelection = function()
  {
    ...
    catch(e)
    {
      ...
      this.tRange = this.tInstance.document.createRange();
//      this.tRange = this.tSelection.getRangeAt(0);
  ...
  this.tLoadImageTable = function()
  {
    ...
    tLinkTableHTML += '  <tr>';
    tLinkTableHTML += '    <td style="width:250px;"><input type="file" class="file" name="post" id="livenotice_upload_file"><span id="livenotice_tips"></span>';
    tLinkTableHTML += '    <input style="display:none;" readonly="readonly" id="' + this.tid + '-ubbEditorImageText" type="text" value="http://" class="ubbEditorImageText" ondblclick="this.select();" /></td>';
    tLinkTableHTML += '  </tr>';
    tLinkTableHTML += '  <tr>';
    ...
    tLinkTableHTML += '</table>';
    this.tLoadMaskShow(tLinkTableHTML);
    $("#livenotice_upload_file").uploadify({
        'buttonText' : '涓婁紶鍥剧墖',
        ...
        'uploader'      : 'index.php?m=Captcha&do=saveLivenoticeImg',
        'onUploadError' : function(file, errorCode, errorMsg, errorString) {
            alert('The file ' + file.name + ' could not be uploaded: ' + errorString);
        },
        'onUploadStart' : function(file) {
//                                                       nEditor.tGetSelection();
        },
        'onUploadSuccess' : function(file, data, response) {
            if (response) {
                ...
                var imgUrl = obj.img;
                var imgKey = obj.file;
                $('#liveNoticeTextarea-ubbEditorImageText').val(imgUrl);
                $('#livenotice_tips').text("涓婁紶鎴愬姛")
            }
        } 
    });
  },
